/**
 * 文件路径：/utils/validator.js
 * 文件说明：数据校验工具，提供表单验证、参数校验等方法
 */
...
const validator = {
      ...
      if (typeof validatorItem === 'string') {
        ...
        if (rule && !rule.test(value, validatorItem.param)) {
          return typeof rule.message === 'function' 
            ? rule.message(validatorItem.param)
            : rule.message
        }
  ...
  recipeRules: {
    ...
    description: [{ rule: 'maxLength', param: 1000 }]
  },
  ...
  userRules: {
    ...
    healthTags: [(value) => {
      if (!Array.isArray(value)) return '健康标签必须是数组'
      if (value.length > 10) return '健康标签最多选择10个'
      return true
    }]
  },
...
// 导出便捷函数（用于页面中直接导入使用）
export const validatePhone = (phone) => validator.quickValidate('phone', phone)
export const validatePassword = (password) => validator.quickValidate('password', password)
export const validateVerifyCode = (code) => validator.quickValidate('verifyCode', code)
export const validateRequired = (value) => validator.quickValidate('required', value)
